<script lang="ts">
	...
	$: if ($navigating) setLink($navigating.to.url.pathname);
...
<nav class:absolute class="w-screen">
	<div class="px-8 flex items-center w-screen justify-between h-20 max-w-screen-xl mx-auto">
		...
		<div class="flex gap-4">
			{#each links as link}
				<a
					...
					class:text-white={text === 'light'}
					class:text-surface-100={text !== 'light'}
		...
		<Button label="Submit an application" color={btnColor} link="/apply" />
	</div>
</nav>
